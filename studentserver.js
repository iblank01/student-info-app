 ...
 *   get:
 *     summary: Search for a student by their last name.
...
app.get('/students/search/:last_name', function (req, res) {
  ...
  glob("students/*.json", null, function (err, files) {
    ...
    // Use Promise.all to ensure all files are read before proceeding
    Promise.all(files.map(file => {
      return new Promise((resolve, reject) => {
        fs.readFile(file, "utf8", function (err, data) {
          if (err) {
            ...
            if (student.last_name.toLowerCase() === lastName.toLowerCase()) {
      ...
      if (foundStudents.length > 0) {
        ...
        res.status(404).send({ "message": "No students found with the given last name." });
      }
    }).catch(err => {
